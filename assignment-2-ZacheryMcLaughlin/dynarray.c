...
int logical_index(struct dynarray* da, int phys){
  assert(da);
  return (phys + da->start) % da->capacity;
}
...
void _dynarray_resize(struct dynarray* da, int new_capacity) {
  ...
  for (int i = 0; i < da->size; i++) {
    new_data[i] = da->data[logical_index(da, i)];
  }
...
void dynarray_insert(struct dynarray* da, void* val) {
   ...
   */
  da->data[logical_index(da, da->size)] = val;
  da->size++;
}
...
void* dynarray_get(struct dynarray* da, int idx) {
  ...
  return da->data[logical_index(da, idx)];
}
...
void dynarray_set(struct dynarray* da, int idx, void* val) {
  ...
  da->data[logical_index(da, idx)] = val;
}

/*
 * This function should indicate whether a given dynamic array is currently empty.
 * Specifically, it should return 1 if the specified array is empty (i.e.
 * contains no elements) and 0 otherwise.
 *
 * Params:
 *   dynarray - the dynamic array whose emptiness is being questioned.  May not be NULL.
 */
int dynarray_isempty(struct dynarray* dynarray) {
  if(dynarray->size == NULL)
  {
    return 1;
  }
  else
