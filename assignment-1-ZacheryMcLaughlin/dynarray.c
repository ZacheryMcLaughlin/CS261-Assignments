...
struct dynarray* dynarray_create() {
  struct dynarray* dynar = (struct dynarray*) malloc(sizeof(struct dynarray));
  dynar->data = (void**) malloc(sizeof(void*)*2);
  dynar->capacity = 2;
  dynar->size = 0;
}
...
void dynarray_insert(struct dynarray* da, void* val) {
  if(da->size == da->capacity){
    ...
    void** temp_data = da->data;
    da->data = (void**) malloc(sizeof(void*) * da->capacity);
    for(int i=0; i<da->size; i++){
      da->data[i] = temp_data[i];
    }
    free(temp_data);
  }
  da->data[da->size] = val;
