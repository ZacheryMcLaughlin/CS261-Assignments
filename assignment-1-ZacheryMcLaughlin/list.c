...
struct list* list_create() {
  struct list* lonk = (struct list*) malloc(sizeof(struct list));
  //lonk->head = (struct link*) malloc(sizeof(struct link));
  //lonk->head->next = NULL;
  return lonk;
}
...
int list_position(struct list* list, void* val, int (*cmp)(void* a, void* b)) {
  struct link* cur = list->head;
  int i = 0;
  //printf("I'm calling list position\n");
  while(cur){
    //printf("Beginning loop\n");
    if(!cmp(val, cur->val)){
      return i;
